// Taken from Michael Ehrmann source code of SunClock
...
float my_floor(float x) { return ((int)x); }
...
float my_cos(float x) { return my_sin(x + (M_PI / 2)); }
...
float my_asin(float x) { return (M_PI / 2) - my_acos(x); }

float my_tan(float x) { return my_sin(x) / my_cos(x); }